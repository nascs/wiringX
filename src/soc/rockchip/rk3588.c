...
#define BUS_IOC_GPIOXX_IOMUX_X       0xffff     // gpioXX iomux, which is nonextent
...
static struct layout_t
{
    char *name;
    int bank;
    int iomux_num; // 0 for PMU1_IOC, 1 for PMU2_IOC, 2 for BUS_IOC
    ...
    {"GPIO3_D6", 3, 2, {CRU_GATE_CON17, 2}, {BUS_IOC_GPIOXX_IOMUX_X,  8}, {GPIO_SWPORT_DDR_H, 14}, {GPIO_SWPORT_DR_H, 14}, {GPIO_EXT_PORT, 30}, FUNCTION_DIGITAL, PINMODE_NOT_SET, 0},
    {"GPIO3_D7", 3, 2, {CRU_GATE_CON17, 2}, {BUS_IOC_GPIOXX_IOMUX_X, 12}, {GPIO_SWPORT_DDR_H, 15}, {GPIO_SWPORT_DR_H, 15}, {GPIO_EXT_PORT, 31}, FUNCTION_DIGITAL, PINMODE_NOT_SET, 0}, 
    ...
    {"GPIO4_D6", 4, 2, {CRU_GATE_CON17, 4}, {BUS_IOC_GPIOXX_IOMUX_X,  8}, {GPIO_SWPORT_DDR_H, 14}, {GPIO_SWPORT_DR_H, 14}, {GPIO_EXT_PORT, 30}, FUNCTION_DIGITAL, PINMODE_NOT_SET, 0},
    {"GPIO4_D7", 4, 2, {CRU_GATE_CON17, 4}, {BUS_IOC_GPIOXX_IOMUX_X, 12}, {GPIO_SWPORT_DDR_H, 15}, {GPIO_SWPORT_DR_H, 15}, {GPIO_EXT_PORT, 31}, FUNCTION_DIGITAL, PINMODE_NOT_SET, 0},
};
...
static int rk3588Setup(void)
{
    ...
    {
        if ((rk3588->gpio[i] = (unsigned char *)mmap(0, rk3588->page_size, PROT_READ | PROT_WRITE, MAP_SHARED, rk3588->fd, rk3588->base_addr[i])) == NULL)
        {
    ...
    if ((cru_ns_register_virtual_address = (unsigned char *)mmap(0, rk3588->page_size, PROT_READ | PROT_WRITE, MAP_SHARED, rk3588->fd, CRU_NS_REGISTER_PHYSICAL_ADDRESS)) == NULL)
    {
        wiringXLog(LOG_ERR, "wiringX failed to map The %s %s CRU memory address", rk3588->brand, rk3588->chip);
        return -1;
    }
    if ((pmu1_ioc_register_virtual_address = (unsigned char *)mmap(0, rk3588->page_size, PROT_READ | PROT_WRITE, MAP_SHARED, rk3588->fd, PMU1_IOC_REGISTER_PHYSICAL_ADDRESS)) == NULL)
    {
        wiringXLog(LOG_ERR, "wiringX failed to map The %s %s CRU memory address", rk3588->brand, rk3588->chip);
        return -1;
    }
    if ((pmu2_ioc_register_virtual_address = (unsigned char *)mmap(0, rk3588->page_size, PROT_READ | PROT_WRITE, MAP_SHARED, rk3588->fd, PMU2_IOC_REGISTER_PHYSICAL_ADDRESS)) == NULL)
    {
        wiringXLog(LOG_ERR, "wiringX failed to map The %s %s CRU memory address", rk3588->brand, rk3588->chip);
        return -1;
    }      
    if ((bus_ioc_register_virtual_address = (unsigned char *)mmap(0, rk3588->page_size, PROT_READ | PROT_WRITE, MAP_SHARED, rk3588->fd, BUS_IOC_REGISTER_PHYSICAL_ADDRESS)) == NULL)
    {
...
struct layout_t * rk3588GetLayout(int i, int* mapping)
{
    ...
    if (mapping == NULL)
    {
        wiringXLog(LOG_ERR, "The %s %s has not yet been mapped", rk3588->brand, rk3588->chip);
        return NULL;
    }
    ...
    if (rk3588->fd <= 0 || rk3588->gpio == NULL)
    {
        wiringXLog(LOG_ERR, "The %s %s has not yet been setup by wiringX", rk3588->brand, rk3588->chip);
        return NULL;
    }

    pin = &rk3588->layout[mapping[i]];
    ...
    if (pin->iomux_num < 0 || pin->iomux_num >= IOMUX_COUNT)
    {
        wiringXLog(LOG_ERR, "pin->bank out of range: %i, expect 0-4", pin->bank);
        return NULL;
    }
...
#define rk3588GetPinLayout(i) (rk3588GetLayout(i, rk3588->map))
#define rk3588GetIrqLayout(i) (rk3588GetLayout(i, rk3588->irq))
...
static int rk3588DigitalWrite(int i, enum digital_value_t value)
{
    ...
    if ((pin = rk3588GetPinLayout(i)) == NULL)
    {
        return -1;
    }
    ...
    {
        REGISTER_CLEAR_BITS(out_reg, pin->out.bit, 2);
    }
    else
    {
...
static int rk3588DigitalRead(int i)
{
    ...
    {
        wiringXLog(LOG_ERR, "The %s %s GPIO%d is not set to input mode", rk3588->brand, rk3588->chip, i);
        return -1;
    }

    in_reg = (volatile unsigned int *)(rk3588->gpio[pin->bank] + pin->in.offset);
    val = *in_reg;
...
static int rk3588PinMode(int i, enum pinmode_t mode)
{
    ...
    if (mode == PINMODE_INPUT) {
        REGISTER_CLEAR_BITS(dir_reg, pin->direction.bit, 2);
...
static int rk3588ISR(int i, enum isr_mode_t mode)
{
    ...
    if ((pin = rk3588GetIrqLayout(i)) == NULL)
    {
        return -1;
    }

    sprintf(path, "/sys/class/gpio/gpio%d", rk3588->irq[i]);
    if ((soc_sysfs_check_gpio(rk3588, path)) == -1)
    {
        sprintf(path, "/sys/class/gpio/export");
        if (soc_sysfs_gpio_export(rk3588, path, rk3588->irq[i]) == -1)
        {
            return -1;
        }
    }

    sprintf(path, "/sys/class/gpio/gpio%d/direction", rk3588->irq[i]);
    if (soc_sysfs_set_gpio_direction(rk3588, path, "in") == -1)
    {
        return -1;
    }

    sprintf(path, "/sys/class/gpio/gpio%d/edge", rk3588->irq[i]);
    if (soc_sysfs_set_gpio_interrupt_mode(rk3588, path, mode) == -1)
    {
        return -1;
    }

    sprintf(path, "/sys/class/gpio/gpio%d/value", rk3588->irq[i]);
    if ((pin->fd = soc_sysfs_gpio_reset_value(rk3588, path)) == -1)
    {
        return -1;
    }
...
static int rk3588WaitForInterrupt(int i, int ms)
{
    ...
    {
        wiringXLog(LOG_ERR, "The %s %s GPIO %d is not set to interrupt mode", rk3588->brand, rk3588->chip, i);
        return -1;
    }

    return soc_wait_for_interrupt(rk3588, pin->fd, ms);
}
...
static int rk3588GC(void)
{
    ...
    if (rk3588->map != NULL)
    {
        for (int i = 0; i < rk3588->map_size; i++)
        {
            pin = &rk3588->layout[rk3588->map[i]];
            if (pin->mode == PINMODE_OUTPUT)
            {
            ...
            {
                sprintf(path, "/sys/class/gpio/gpio%d", rk3588->irq[i]);
                if ((soc_sysfs_check_gpio(rk3588, path)) == 0)
                {
                    sprintf(path, "/sys/class/gpio/unexport");
                    soc_sysfs_gpio_unexport(rk3588, path, rk3588->irq[i]);
                }
            }
